`timescale 1ns / 1ps
//////////////////////////////////////////////////////////////////////////////////
// pixel_generator_6.v  -  Stage 6: divider-integrated, board-interface Newton engine.
//
...
//
// STAGE 6 CHANGE: each lane's two combinational divides (dr, di) are replaced by
// two multi-cycle divider_seq instances. The lane FSM latches the divide inputs
// (denom, numerators) in S_CALC, pulses start once entering S_DIV, stalls until
// both quotients are ready, then commits in S_UPD. This removes the long
// combinational divide from the critical path so Vivado can close timing at a
// higher Fmax. Output is unchanged (divider_seq is bit-exact vs '/'); per-pixel
// CYCLE COUNT rises (each divide takes WIDTH+1 cycles) - the expected
// cycles-for-Fmax trade, with the net wall-clock decided by Vivado Fmax.
//
...
// ---- Per-lane state ----
localparam S_INIT = 3'd0;   // load pixel coordinate
localparam S_CALC = 3'd1;   // compute & latch denom/numerators; check singularity
localparam S_DIV  = 3'd2;   // run the two dividers, wait for done
localparam S_UPD  = 3'd3;   // commit z_next, test convergence / iter cap
localparam S_DONE = 3'd4;   // hold colour, wait for packer handshake

reg  signed [63:0] zr   [0:LANES-1];
reg  signed [63:0] zi   [0:LANES-1];
reg  [5:0]         iter [0:LANES-1];
reg  [1:0]         ridx [0:LANES-1];
reg  [2:0]         lst  [0:LANES-1];   // lane state
...
// ---- Per-lane Newton datapath + FSM (generate), Stage 6 divider-integrated ----
genvar g;
generate
  for (g = 0; g < LANES; g = g + 1) begin : lane
    // --- combinational Newton numerator/denominator (NO divide here) ---
    ...
    wire signed [63:0] denom_c = (fpr*fpr)/SCALE + (fpi*fpi)/SCALE;
    wire signed [63:0] numr_c  = (fr*fpr)/SCALE + (fi*fpi)/SCALE;
    wire signed [63:0] numi_c  = (fi*fpr)/SCALE - (fr*fpi)/SCALE;
    // numerators pre-scaled by SCALE so the quotient is the Q12 step value,
    // exactly matching the old (numr*SCALE)/denom.
    wire signed [63:0] numr_s_c = numr_c * SCALE;
    wire signed [63:0] numi_s_c = numi_c * SCALE;

    // --- registered divide inputs (stable for the whole multi-cycle divide) ---
    reg  signed [63:0] denom_r, numr_r, numi_r;

    // --- two multi-cycle dividers per lane (share denom, different numerators) ---
    reg               div_start;
    wire signed [63:0] q_dr, q_di;
    wire              done_r, done_i, busy_r, busy_i;

    divider_seq #(.WIDTH(64)) u_div_r (
        .clk(out_stream_aclk), .rstn(periph_resetn),
        .start(div_start), .numer(numr_r), .denom(denom_r),
        .quot(q_dr), .done(done_r), .busy(busy_r) );
    divider_seq #(.WIDTH(64)) u_div_i (
        .clk(out_stream_aclk), .rstn(periph_resetn),
        .start(div_start), .numer(numi_r), .denom(denom_r),
        .quot(q_di), .done(done_i), .busy(busy_i) );

    // latch each quotient as its divider finishes; cleared each S_CALC
    reg signed [63:0] dr_l, di_l;
    reg               got_r, got_i;
    always @(posedge out_stream_aclk) begin
        if (!periph_resetn) begin
            got_r <= 1'b0; got_i <= 1'b0;
        end else if (lst[g] == S_CALC) begin
            got_r <= 1'b0; got_i <= 1'b0;          // fresh divide per iteration
        end else begin
            if (done_r) begin dr_l <= q_dr; got_r <= 1'b1; end
            if (done_i) begin di_l <= q_di; got_i <= 1'b1; end
        end
    end
    wire div_done = got_r & got_i;

    // z_next from the latched quotients (committed in S_UPD)
    wire signed [63:0] zr_n = zr[g] - dr_l;
    wire signed [63:0] zi_n = zi[g] - di_l;
    ...
    always @(posedge out_stream_aclk) begin
        if (!periph_resetn || frame_restart) begin
            ...
            div_start <= 1'b0;                      // default: not pulsing
            case (lst[g])
                S_INIT: begin
                ...
                S_CALC: begin
                    // zr/zi are stable now: latch divide inputs
                    denom_r <= denom_c;
                    numr_r  <= numr_s_c;
                    numi_r  <= numi_s_c;
                    if (denom_c == 0) begin
                        ridx[g] <= 2'd3;            // singularity -> black
                        lst[g]  <= S_DONE;
                    end else begin
                        div_start <= 1'b1;          // one-cycle start pulse
                        lst[g]    <= S_DIV;
                    end
                end
                S_DIV: begin
                    // div_start was asserted entering here; hold until both done
                    if (div_done) lst[g] <= S_UPD;
                end
                S_UPD: begin
                    if (anyc) begin
