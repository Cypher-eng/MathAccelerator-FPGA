...
def root_id(zr, zi):
    for k, (rr, ri) in enumerate(ROOTS):
        dr = zr - rr
        di = zi - ri
        if dr < TOL and dr > -TOL and di < TOL and di > -TOL:
            return k
    return 3
...
def pixel(px, py, zr0, zi0, step, maxit):
    ...
    for it in range(maxit):
        zr2 = tdiv(zr * zr, SCALE) - tdiv(zi * zi, SCALE)
        zi2 = tdiv(2 * zr * zi, SCALE)
        zr3 = tdiv(zr2 * zr, SCALE) - tdiv(zi2 * zi, SCALE)
        zi3 = tdiv(zr2 * zi, SCALE) + tdiv(zi2 * zr, SCALE)
        fr = zr3 - SCALE
        fi = zi3
        fpr = 3 * zr2
        fpi = 3 * zi2
        denom = tdiv(fpr * fpr, SCALE) + tdiv(fpi * fpi, SCALE)
        if denom == 0:
            return (0, 0, 0)
        numr = tdiv(fr * fpr, SCALE) + tdiv(fi * fpi, SCALE)
        numi = tdiv(fi * fpr, SCALE) - tdiv(fr * fpi, SCALE)
